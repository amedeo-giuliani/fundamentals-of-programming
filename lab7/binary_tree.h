...
private:
	...
	void traverse_levelorder_NR(Node<T>* theRoot)
	 ...
	 int equal_tree_structure(Node<T>* theRootA, Node<T>* theRootB){
		...
		if (theRootA == NULL){
			...
			r = equal_tree_structure(theRootA->lChildptr, theRootB->lChildptr);
			
			if(r == 0)
				return 0;

			r = equal_tree_structure(theRootA->rChildptr, theRootB->rChildptr);
	 ...
	 int equal_trees(Node<T>* theRootA, Node <T>* theRootB){
		if (theRootA == NULL){
			if(theRootB != NULL)
				return 0;
		}
			...
			if(theRootA->data != theRootB->data)
				return 0;

			int r;

			r = equal_trees(theRootA->lChildptr, theRootB->lChildptr);
			
			if(r == 0)
				return 0;

			r = equal_trees(theRootA->rChildptr, theRootB->rChildptr);
    ...
    public:
		...
		int equal_structure(Node<T>* bt2){
			return equal_tree_structure(root,bt2);
		}

		int equal(Node<T>* bt2){
			return equal_trees(root,bt2);
		}
